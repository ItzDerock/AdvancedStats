...
const method = new BaseMethod(config.database.influx.bucket, 'bot', 'memberCount');

module.exports.get = function(start, end, mean = false) {
    return method.get(start, { 
        ends: end, 
        additionalQueryParams: (qb) => mean ? qb.mean() : qb
    });
}

module.exports.set = function(count, forceSave = false) {
    return method.set(count, {}, forceSave)
}
